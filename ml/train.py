...
from ml.model import BiLSTMWithAttention, BiLSTMWithAttentionAndKFold, get_pos_weight
from ml.training import train_model, evaluate, train_with_kfold
...
def parse_args():
    ...
    parser.add_argument('--use_kfold', action='store_true',
                        help='Использовать K-fold валидацию')
    parser.add_argument('--n_splits', type=int, default=5,
                        help='Количество разбиений для K-fold валидации')
...
def main():
    ...
    # Проверяем, используем ли K-fold валидацию
    if args.use_kfold:
        print("Используем K-fold валидацию...")
        
        # Подготовка данных
        # Для K-fold нам не нужно разделять на обучающую и валидационную выборки
        # так как это будет делать KFold
        if args.balance_method != 'none':
            print(f"Балансировка данных с использованием метода: {args.balance_method}...")
            texts, meta_features, labels = balance_dataset(
                texts, np.array(meta_features), labels, method=args.balance_method
            )
        ...
        full_dataset = DepressionDataset(
            texts, meta_features, labels, ft_model=ft, max_len=args.max_len
        )
        
        # Фабрики для создания модели и оптимизатора
        def model_factory():
            return BiLSTMWithAttentionAndKFold(
        ...
        print("Начало обучения с K-fold валидацией...")
        start_time = datetime.now()
        
        model, fold_metrics = train_with_kfold(
            ...
            base_model_name=args.model_name.replace('.pt', '')
        )
        ...
        with open(os.path.join(args.output_dir, 'kfold_results.json'), 'w', encoding='utf-8') as f:
            json.dump(results, f, ensure_ascii=False, indent=4)
        
        print(f"Результаты сохранены в {os.path.join(args.output_dir, 'kfold_results.json')}")
